...
import { assets, dummyDashboardData, type DummyDashboard } from "../../assets/assets";
    ...
    <div className="px-4 pt-10 md:px-10 flex flex-col">
      ...
      <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4  gap-6 my-8 max-w-4xl">
        {dashboardCard.map((card, index) => (
          <div key={index} className="flex gap-2 items-center justify-between p-4 border border-border-color rounded">
            <div>
      ...
      <div className="flex flex-wrap items-start gap-6 mb-8 w-full">
        {/* Recent Bookings */}
        <div className="p-4 md:p-6 border border-border-color rounded w-full max-w-lg">
          ...
          {data.recentBookings.map((booking, index) => (
            <div key={index} className="flex justify-between items-center mt-4">
              <div className="flex items-center gap-2">
                <div className="hidden md:flex items-center justify-center w-12 h-12 rounded bg-primary/10">
                  <img src={assets.listIconColored} alt='icon' className="w-5 h-5" />
                </div>
                <div>
              ...
              <div className="flex items-center gap-2 font-medium">
                <p className="text-sm text-gray-500">{currencySymbol}{booking.price}</p>
                <p className="px-3 py-0.5 border border-border-color rounded text-sm">{booking.status}</p>
              </div>
            </div>
          ))}
        ...
        {/* Monthly Revenue */}
        <div className="p-4 md:p-6 mb-6 border border-border-color rounded w-full md:max-w-xs">
          <h1 className="text-lg font-medium">Monthly Revenue</h1>
          <p className="text-gray-500">Revenue for current month</p>
          <p className="text-3xl mt-6 font-semibold text-primary">{currencySymbol}{data.monthlyRevenue}</p>
        </div>
      </div>
    </div>
  )
